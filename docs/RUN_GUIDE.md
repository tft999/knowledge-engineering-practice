...
常见硬约束语句也可以由Agent的规则解析兜底；开放表达和GraphRAG回答需要配置模型。
...
```powershell
$body = @{
  question = "我有鸡蛋、洋葱和面包片，不吃辣，做两道菜，最多补购两种。"
  top_k = 5
} | ConvertTo-Json
Invoke-RestMethod -Method Post -Uri http://127.0.0.1:8000/api/v1/agent `
  -ContentType "application/json" -Body $body
```

返回的 `mode` 为 `planner`、`graphrag`、`help` 或 `clarification`，分别对应菜单结果、
带引用回答、使用帮助和需要用户确认的歧义输入。
...
硬约束问题应显示标准化词语、Agent执行摘要、均衡菜单和多样性说明；知识问题应显示
检索器名称、路由原因、`[E1]` 引用和固定版本原文链接。
...
进入“智能问答”，先展示自然语言硬约束，再展示GraphRAG：
