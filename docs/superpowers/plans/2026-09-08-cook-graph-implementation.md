...
## 工作清单
- [ ] 写入设计与计划、配置独立虚拟环境、工程入口和 Git 忽略规则。
- [ ] 先编写解析与归一化测试，确认缺失实现失败后实现数据处理。
- [ ] 固定提交下载，排除模板；生成来源清单、JSONL、图谱与质量报告。
- [ ] 逐份阅读并审核至少 10 道真实菜谱，以源文件哈希保护审核有效性。
- [ ] 实现 1—3 道菜推荐、共享缺料去重、可选项与排除项处理。
- [ ] 小型人工图与独立穷举结果核对；CLI 提供文本与 JSON 输出。
- [ ] 实现 Neo4j 幂等导入与快照验证；离线测试和在线验收分开记录。
- [ ] 全量运行、测试与 Ruff，通过后更新 README 与验收报告，阶段性本地提交。
