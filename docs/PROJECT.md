...
当前版本已经能演示联合补购、食材类别排除、封闭食材选择组、FastAPI 和 Web UI。
开放选择组和复合食材推理仍是后续目标。当前已实现 GraphRAG 的检索、路由、引用检查、
问答 API 和页面；真实模型演示与最终实验仍以配置状态和人工评测结果为准。
...
- 10 道菜的内容哈希绑定人工审核；
- 100 道来源绑定的 v2 AI 辅助标注草稿、262 个食材、42 个选择组和人工审核工具；
- v2 技术投影中 53 道草稿可用于接口联调，但不计作人工审核成果；
- 1 至 3 道菜的联合补购优化；
- 已有食材、常备调料、具体食材或审核类别排除及最多补购种类约束；
- 版本化人工辣椒分类、多级后代展开和稳定排除解释路径；
- 封闭食材选择组求解：优先库存成员，开放组和工具组继续阻断；
- FastAPI 健康检查、推荐、菜谱详情和局部图谱接口；
- React 单页规划器、被排除菜谱图谱与真实浏览器联调；
- 中文文本与 JSON CLI；
- Neo4j 快照隔离导入及节点、边、属性一致性验证；
- 99 个 Python 测试和 17 个前端测试通过，模拟及真实 API 浏览器流程通过；
- 本次整合环境未连接 Neo4j，2 个在线测试跳过，成员的数据分支另有在线验收记录。

### 5.2 最终阶段：数据、GraphRAG 与实验

- 将人工审核菜谱扩展到不少于 100 道；
- 完成人工审核，使 v2 中的 `requirement` 与 `review_state` 真正进入严格数据；
- 支持开放食材选择组和工具选择组；
- 扩展食材类别层级和经审核的有限成分关系；
- 增加审核工作流和数据质量统计页面；
- 完成真实 Neo4j 在线幂等导入验证；
- 建立不少于 100 条人工评测输入；
- 从审核菜谱生成稳定 Evidence，并提供四种检索器；
- 将局部图关系和原文一起提供给模型，生成并校验 `[E1]` 引用；
- 在 Web 页面展示路由、证据和证据不足状态。
...
- 完成 Vector RAG 与 HybridCypher GraphRAG 对照，报告 Recall@5、答案与引用指标；
- 完成带引用的菜谱步骤与关系问答，且不影响确定性菜单功能。
...
`ONE_OF` 已进入 v2 图和技术联调投影；严格推荐仍只使用 10 道人工审核数据。
`HAS_COMPONENT` 仅保留在 v2 标准图中，尚未参与生产推理。
