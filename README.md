...
需要直接准备汇报环境时，可按[启动与演示指南](docs/RUN_GUIDE.md)分别启动演示数据、
真实菜单后端或完整 GraphRAG 问答。
...
基础数据集包含10道逐道审核菜谱；v2 数据集的100道菜已在源哈希、逐字证据、本体端点、
选择组和图结构自动复核通过后，由组长于2026-09-14批量批准。100道均记录审核状态，
其中60道满足严格数据条件；39道保留原文冲突或分支问题，另有不满足分类条件的记录，
这些记录虽已检查但不会进入严格推荐。当前后端兼容投影可安全使用53道菜。

审核记录与源文件、标注内容双哈希绑定；任一内容变化都会使批准失效。审核方式属于时间受限的
“机器全量核验＋组长批量批准”，没有进行双人逐道独立复核，汇报时应按这一口径说明。
...
问答页面同时提供 `POST /api/v1/agent`。Agent 将自然语言路由到确定性菜单规划器或
GraphRAG：例如“我有番茄和鸡蛋，不吃辣，做两道菜”会先完成实体标准化，再执行类别
排除、联合补购和均衡排序；“你好”直接返回使用帮助，不会进入向量检索。模型只能选择
预定义工具并提出实体候选，最终实体必须存在于别名字典或知识图谱中。

菜单规划默认在 `max_buy` 硬上限内优先减少重复核心食材和重复菜品类别，再比较补购数和
库存覆盖。盐、油、常用酱料及葱姜蒜仍参与库存与补购计算，但不会仅因共同使用这些调味料
而被判定为相似菜。类别为 `condiment` 的酱料和调味品制作记录不会作为一道独立菜单菜品。
...
选择组、工具、类别、结构化用量、加工形态和逐行证据。100道均通过源哈希与逐字引文校验，
并已生成绑定源哈希和标注哈希的组长批准记录；
39道含待裁决问题，因此保留在完整图谱中但不进入严格推荐。
完整 v2 图谱保留全部语义；另生成与当前推荐 API 兼容的 `backend-graph.json`。后端已支持
封闭食材选择组，当前有53道菜可用于后端和GraphRAG联调；其余记录因来源/标注问题、
缺少公开菜名或工具选择组被明确阻断。
